...
function generateHtmlPage(movies: Movie[], moviesByDay: Map<string, Movie[]>): void {
    ...
    for (const [day, channelMovies] of moviesByDayAndChannel) {
        ...
        for (const [channel, channelMovieList] of channelMovies) {
            ...
            channelMovieList.forEach((movie, idx) => {
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        header { background: linear-gradient(135deg, var(--gradient-start), var(--gradient-end)); padding: 0; text-align: center; position: sticky; top: 0; z-index: 100; box-shadow: 0 4px 20px rgba(0,0,0,0.5); }
        header h1 { font-size: 2.5rem; margin-bottom: 0.5rem; }
        header p { font-size: 1.1rem; opacity: 0.9; }
        .stats { display: flex; justify-content: center; gap: 1rem; margin-top: 1rem; flex-wrap: wrap; align-items: center; }
        ...
        @media (max-width: 768px) { header h1 { font-size: 1.8rem; } main { padding: 1rem; } .movies-grid { grid-template-columns: 1fr; } .toast-container { left: 1rem; right: 1rem; } }
    </style>
</head>
<body>
    <header>
    ...
    <script>
        const API_URL = 'http://localhost:3000/api/freebox';
